...
.post('/api/orders', async (c) => {
  ...
  .get('/api/orders/table/:table_id', async (c) => {
  ...
  .patch('/api/orders/:id/status', async (c) => {
    const id = c.req.param('id')
  ...
  .get('/api/revenue/history', async (c) => {
  ...
  .get('/api/revenue/monthly', async (c) => {
    const date = c.req.query('date') || 'now';
